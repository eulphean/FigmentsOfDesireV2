...
void Azra::createMesh(AgentProperties agentProps) {
  ...
  float sizeRatio = faceArea/(PI * faceRadius * faceRadius);
  
  // Calculate # of meshPoints and set the variable to that.
  faceCircumference = 2 * PI * faceRadius; // circumference
  
  float totalJointLength = faceCircumference/softJointLength; // jointLength is the percent (Note this length will come from the GUI based on some properties. This needs to be in good ratio with the radius.
  meshPoints = (faceCircumference - totalJointLength) / (agentProps.vertexRadius * 2);  // Total number of mesh points.
  ...
  for(int i = 1; i < meshPoints; i++){
    ...
    // Add the required mesh vertex.
    mesh.addVertex({meshOrigin.x + (x * faceRadius), meshOrigin.y + y * faceRadius, 0});
    
    // Add the mesh texture coordinate.
    mesh.addTexCoord(glm::vec2(faceWidth/2 + x * faceRadius * sizeRatio, faceHeight/2 + y * faceRadius * sizeRatio));
  }
}
